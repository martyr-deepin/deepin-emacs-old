...
let g:haskell_refac_version = "HaRe 27/03/2008"

if !exists("g:haskell_refac_refactorer")
	let g:haskell_refac_refactorer = "/home/andy/MyEmacs/Site-Lisp/Packages/HaRe/refactorer/pfe"
endif

if !exists("g:haskell_refac_refactorer_client")
	let g:haskell_refac_refactorer_client = "/home/andy/MyEmacs/Site-Lisp/Packages/HaRe/refactorer/pfe_client"
endif

if !exists("g:haskell_refac_chasePaths")
	let g:haskell_refac_chasePaths = getcwd() . " /home/andy/MyEmacs/Site-Lisp/Packages/HaRe/tools/base/tests/HaskellLibraries"
endif
...
function! Haskell_refac_pwToPf()
	let fileName0 = expand("%:p")
	let start1 = line("'<")." ".col("'<")
	let end1 = line("'>")." ".col("'>")
call Haskell_refac_client_cmd(" pwToPf"." ".fileName0." ".start1." ".end1)
endfunction
...
function! Haskell_refac_refacDataNewType()
	let fileName0 = expand("%:p")
	let line1 = line(".")
	let column1 = col(".")

call Haskell_refac_client_cmd(" refacDataNewType"." ".fileName0." ".line1." ".column1)
endfunction
...
command! PwToPf :echo Haskell_refac_pwToPf()
...
command! RefacDataNewType :echo Haskell_refac_refacDataNewType()
...
if has("menu")
	...
	amenu Haskell(Refac).Definitions.Unfold\ def :UnfoldDef<cr>
	tmenu Haskell(Refac).Definitions.Unfold\ def <fileName> <line> <column> Unfold a definition
	...
	amenu Haskell(Refac).Definitions.Fold\ Definition :SubFunctionDef<cr>
	tmenu Haskell(Refac).Definitions.Fold\ Definition <fileName> <line> <column> <line> <column> Folds against a definition
	amenu Haskell(Refac).Definitions.Transform\ pointwise\ to\ pointfree :PwToPf<cr>
	tmenu Haskell(Refac).Definitions.Transform\ pointwise\ to\ pointfree <fileName> <line> <column> <line> <column> Tries to convert a pointwise into a pointfree expression
	...
	amenu Haskell(Refac).Data\ types.Convert\ a\ data\ type\ into\ a\ newtype :RefacDataNewType<cr>
	tmenu Haskell(Refac).Data\ types.Convert\ a\ data\ type\ into\ a\ newtype <fileName> <line> <column> Transforms a data type into a new type
	...
	amenu Haskell(Refac).Misc.Convert\ pattern\ to\ use\ an\ as\ pattern :RefacAsPatterns<cr>
	tmenu Haskell(Refac).Misc.Convert\ pattern\ to\ use\ an\ as\ pattern <fileName> <name (Name for Pattern: )> <line> <column> <line> <column> Converts all appropriate patterns to use an as binding.
	amenu Haskell(Refac).Misc.Unfold\ references\ to\ AS\ patterns :RefacUnfoldAsPatterns<cr>
	tmenu Haskell(Refac).Misc.Unfold\ references\ to\ AS\ patterns <fileName> <line> <column> <line> <column> Converts all references to an as pattern to the actuall pattern.
