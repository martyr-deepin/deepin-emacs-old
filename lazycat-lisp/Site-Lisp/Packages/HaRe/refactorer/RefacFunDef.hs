       ...
       where 
         subExp exp@((Exp _)::HsExpP)
          | sameOccurrence exp e == False
              ...
              {- = do 
                 result <- (callZipHighlightedMatch [] [] e exp)
...
rewritePatsInEnv [] _ = error "empty environment"
...
callZipHighlightedMatch pats w (exp1::HsExpP) (exp2::HsExpP)
  = do zippedE1 <- zipHighlightedMatch pats w exp1 exp2
       return zippedE1
...
zipHighlightedMatch :: Monad m =>  FunctionPats -> WhereDecls 
                    -> HsExpP -> HsExpP -> m NonSetEnv
zipHighlightedMatch pats w (exp1@(Exp (HsId i1))::HsExpP) 
...
zipHighlightedMatch pats w (exp1@(Exp (HsId i))::HsExpP) (exp2::HsExpP)
...
zipHighlightedMatch pats w (exp1@(Exp (HsLit _ i1))::HsExpP)
              (exp2@(Exp (HsLit _ i2))::HsExpP)
  | i1 == i2  = return [ Just (exp1, exp2) ]
  | otherwise = return [ Nothing ]

zipHighlightedMatch pats w (exp1@(Exp (HsLit _ _))::HsExpP) (exp2::HsExpP)
  = return [Nothing]

zipHighlightedMatch pats w (exp1@(Exp (HsInfixApp e1 i e2))::HsExpP)
              (exp2@(Exp (HsInfixApp e11 i2 e22))::HsExpP)
  | i == i2   = do zippedE1 <- zipHighlightedMatch pats w e1 e11
                   zippedE2 <- zipHighlightedMatch pats w e2 e22
...
zipHighlightedMatch pats w (exp1@(Exp (HsApp e1 e2))::HsExpP)
              (exp2@(Exp (HsApp e11 e22))::HsExpP)
  = do zippedE1 <- zipHighlightedMatch pats w e1 e11
       zippedE2 <- zipHighlightedMatch pats w e2 e22
       return (zippedE1 ++ zippedE2)

zipHighlightedMatch pats w (exp1@(Exp (HsApp e1 e2))::HsExpP) (exp2::HsExpP)
  = return [Nothing]

zipHighlightedMatch pats w (exp1@(Exp (HsNegApp _ e1))::HsExpP)
              (exp2@(Exp (HsNegApp _ e2))::HsExpP)
  = do zippedE1 <- zipHighlightedMatch pats w e1 e2
       return zippedE1

zipHighlightedMatch pats w (exp1@(Exp (HsNegApp _ e1))::HsExpP) (exp2::HsExpP)
  = return [Nothing]

zipHighlightedMatch pats w (exp1@(Exp (HsLambda _ e1))::HsExpP)
              (exp2@(Exp (HsLambda _ e11))::HsExpP)
  = do zippedE1 <- zipHighlightedMatch pats w e1 e11
       return zippedE1

zipHighlightedMatch pats w (exp1@(Exp (HsLet _ e1))::HsExpP)
              (exp2@(Exp (HsLet _ e11))::HsExpP)
  = do zippedE1 <- zipHighlightedMatch pats w e1 e11
       return zippedE1

zipHighlightedMatch pats w (exp@(Exp (HsLet _ e1))::HsExpP) (exp2::HsExpP)
  = fail ""

zipHighlightedMatch pats w (exp1@(Exp (HsLambda _ e1))::HsExpP) (exp2::HsExpP)
  = return [Nothing]

zipHighlightedMatch pats w (exp1@(Exp (HsIf e1 e2 e3))::HsExpP)
              (exp2@(Exp (HsIf e11 e22 e33))::HsExpP)
  = do zippedE1 <- zipHighlightedMatch pats w e1 e11
       zippedE2 <- zipHighlightedMatch pats w e2 e22
       zippedE3 <- zipHighlightedMatch pats w e3 e33
       return (zippedE1 ++ zippedE2 ++ zippedE3)

zipHighlightedMatch pats w (exp1@(Exp (HsIf e1 e2 e3))::HsExpP) (exp2::HsExpP)
  = return [Nothing]

zipHighlightedMatch pats 
                    w 
                    (exp1@(Exp (HsCase e1 ((HsAlt _ e2 p1 ds1):alts1)))::HsExpP)
                    (exp2@(Exp 
                              (HsCase e11 
                                         ((HsAlt _ e22 p11 ds11):alts2)
                              ))::HsExpP)
  = do zippedE1 <- zipHighlightedMatch pats w e1 e11
       exp1     <- extractHsBody p1
       exp2     <- extractHsBody p11
       zippedE2 <- zipHighlightedMatch pats w exp1 exp2
       return (zippedE1 ++ zippedE2)

zipHighlightedMatch pats w (exp1@(Exp 
                                     (HsCase e1 
...
zipHighlightedMatch pats w (exp1@(Exp (HsDo (stat1)))::HsExpP)
              (exp2@(Exp (HsDo (stat2)))::HsExpP)
  = do zippedE1 <- zipHighlightedStmt pats w stat1 stat2
       return zippedE1

zipHighlightedMatch pats w (exp1@(Exp (HsDo (stat1)))::HsExpP) (exp2::HsExpP)
  = return [Nothing]
  
zipHighlightedMatch pats w (exp1@(Exp (HsTuple ([e1])))::HsExpP)
                           (exp2@(Exp (HsTuple ([e11])))::HsExpP)
  = do zippedE1 <- zipHighlightedMatch pats w e1 e11
       return zippedE1

zipHighlightedMatch pats w (exp1@(Exp (HsTuple (e1:e1s)))::HsExpP)
  ...
  = do zippedE1 <- zipHighlightedMatch pats w e1 e11
       ...
       where
         ...
         zippedE2 [] _ = return [Nothing]
         zippedE2 (x:xs) (y:ys)
          = do result1 <- zipHighlightedMatch pats w x y
...
zipHighlightedMatch pats w (exp1@(Exp (HsList ([e1])))::HsExpP)
                           (exp2@(Exp (HsList ([e11])))::HsExpP)
  = do zippedE1 <- zipHighlightedMatch pats w e1 e11
       return zippedE1

zipHighlightedMatch pats w (exp1@(Exp (HsList (e1:e1s)))::HsExpP)
  ...
  = do zippedE1 <- zipHighlightedMatch pats w e1 e11
       ...
       where
         ...
         result1 <- zipHighlightedMatch pats w x y
         result2  <- zippedE2 xs ys
         if result1 == [Nothing] || result2 == [Nothing] 
           then return [Nothing]
           else return (result1 ++ result2)

zipHighlightedMatch pats w (exp1@(Exp (HsList (e1:e1s)))::HsExpP) (exp2::HsExpP)
  = return [Nothing]

zipHighlightedMatch pats w (exp1@(Exp (HsParen e1))::HsExpP)
              (exp2@(Exp (HsParen e11))::HsExpP)
  = do zippedE1 <- zipHighlightedMatch pats w e1 e11
       return zippedE1

zipHighlightedMatch pats w (exp1@(Exp (HsParen e1))::HsExpP) (exp2::HsExpP)
  = return [Nothing]

zipHighlightedMatch pats w (exp1@(Exp (HsLeftSection e1 i1))::HsExpP)
              (exp2@(Exp (HsLeftSection e11 i2))::HsExpP)
  | i1 == i2  = do zippedE1 <- zipHighlightedMatch pats w e1 e11
                   return zippedE1
  | otherwise = return [Nothing]
...
zipHighlightedMatch pats w (exp1@(Exp (HsRightSection i1 e1))::HsExpP)
...
zipHighlightedMatch pats w (exp1@(Exp (HsRecConstr _ i1 i2 ))::HsExpP)
...
zipHighlightedMatch pats w (exp1@(Exp (HsRecUpdate _ e1 i1))::HsExpP)
...
zipHighlightedMatch pats w (exp1@(Exp (HsEnumFrom e1))::HsExpP)
              (exp2@(Exp (HsEnumFrom e11))::HsExpP)
  = do zippedE1 <- zipHighlightedMatch pats w e1 e11
       return zippedE1

zipHighlightedMatch pats w (exp1@(Exp (HsEnumFrom e1))::HsExpP) (exp2::HsExpP)
  = return [Nothing]

zipHighlightedMatch pats w (exp1@(Exp (HsEnumFromTo e1 e2))::HsExpP)
...
zipHighlightedMatch pats w (exp1@(Exp (HsEnumFromThen e1 e2))::HsExpP)
...
zipHighlightedMatch pats w (exp1@(Exp (HsEnumFromThenTo e1 e2 e3))::HsExpP)
...
zipHighlightedMatch pats w (exp1@(Exp (HsListComp (stat1)))::HsExpP)
...
zipHighlightedMatch pats w (exp1@(Exp (HsExpTypeSig _ e1 c1 t1))::HsExpP)
...
zipHighlightedMatch pats w (exp1@(Exp (HsAsPat i1 e1))::HsExpP)
              (exp2@(Exp (HsAsPat i11 e11))::HsExpP)
  | i1 == i11 = do zippedE1 <- zipHighlightedMatch pats w e1 e11
                   return zippedE1
  | otherwise = return [Nothing]

zipHighlightedMatch pats w (exp1@(Exp (HsAsPat i1 e1))::HsExpP) (exp2::HsExpP)
  = return [Nothing]

zipHighlightedMatch pats w (exp1@(Exp (HsIrrPat e1))::HsExpP)
              (exp2@(Exp (HsIrrPat e11))::HsExpP)
  = do zippedE1 <- zipHighlightedMatch pats w e1 e11
       return zippedE1

zipHighlightedMatch pats w (exp1@(Exp (HsIrrPat e1))::HsExpP) (exp2::HsExpP)
  = return [Nothing]

zipHighlightedMatch pats w (exp1::HsExpP) (exp2::HsExpP)
  = return [Nothing]
...
zipHighlightedField pats w ((fiel1@(HsField i1 e1)):xs)
  ...
  = if i1 == i11
      then do zippedE1 <- zipHighlightedMatch pats w e1 e11
              return zippedE1
      else do
              return [Nothing]
...
checkInPat (pat1@(Pat (HsPId (HsVar (PNT (PN (UnQual i) (_) ) _ _ )))):pats)
