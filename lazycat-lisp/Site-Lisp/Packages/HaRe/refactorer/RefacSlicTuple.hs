...
import Maybe
import List
import RefacUtils hiding (createApplication)
...
createDecl (d3@(Dec (HsFunBind _ (m@(HsMatch l i1 ps (HsBody e) ds):ms) )):ds3) t@(Dec (HsPatBind loc2 p2 (HsBody e2) ds2)) (True:xs) count a
 = do
      ...
      let (jj,ii) = span (definesMatchOrPat (pNTtoPN i1)) (d3:ds3)
      ...
      return ((Dec (HsFunBind loc )):rest)
       where
        ...
        removeDeadCaseArgs2 pnt (d@(Dec (HsPatBind loc p (HsBody e) ds)):dss) decs
         = case p of
            (Pat (HsPTuple s (l:ls))) -> if pntPrefixOfExpr pnt e
                                           then do
       ...
       where
        ...
        removeFromExp [(Exp (HsTuple (e:es)))] [p@(Pat (HsPId (HsVar (PNT (PN (UnQual x2) s) _ _))))] (d:ds)
           = do
               ...
               -- error $ show (x2, dec, [p])
               if definedInPattern (PN (UnQual x2) s) [p] dec
                then do
                  -- error $ show e
                  return e  
                else do
                  -- error $ show (definedInPattern (PN (UnQual x2) s) [p] dec)
        ...
        removeFromExp [(Exp (HsTuple (e:es)))] (p@(Pat (HsPId (HsVar (PNT (PN (UnQual x2) s) _ _)))):ps) (d:ds)
                     = do
                         ...
                         -- error $ show dec
                         let pns = definedInPatternPName (PN (UnQual x2) s) (p:ps) dec
                         if defaultPN `elem` pns
                          then do
                            ...
                            -- error $ show (x2, dec)
                           if and (map ( (\a b c d -> a d b c)  definedInPattern (p:ps) dec) (hsPNs (p:ps)))
                            then do
                             rest <- removeFromExp2 es ps (d:ds)
                             return (Exp (HsTuple (e:rest)))
                         
                       {-  if definedInPattern (PN (UnQual x2) s) dec
                          then do
                           rest <- removeFromExp2 es ps (d:ds) 
                           return (Exp (HsTuple (e:rest)))   -}
                            else do
        ...
        removeFromExp2 (e@(Exp _):es) (p@(Pat (HsPId (HsVar (PNT (PN (UnQual x2) s) _ _)))):ps) (d:ds)
                     = do
                          let dec = head (findDec e (d:ds) )
                          if definedInPattern (PN (UnQual x2) s) (p:ps) dec
                           then do
                            -- error $ show (x2, dec)
                            rest <- removeFromExp2 es ps (d:ds)
                            return (e:rest)   
           ...
           return ((Dec (HsFunBind loc res)): rest)
            where
             ...
             removeDeadPats2 a dec pnt (d@(Dec (HsPatBind loc p (HsBody e) ds)):dss)
              = case p of
                  (Pat (HsPTuple s (l:ls))) -> if pntPrefixOfExpr2 pnt e
                                                  then do
                    ...
                    removeDeadAlts ((HsAlt s p (HsBody e) ds):as)
                     = 
                      ...
                      (Pat (HsPTuple s2 (l:ls))) ->
                       do
                         if pntPrefixOfExpr pnt e1 && (length (l:ls) == length b)
                           then do
