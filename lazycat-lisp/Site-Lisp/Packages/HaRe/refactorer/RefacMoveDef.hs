  ...
  = if isLocalFunOrPatName pn mod     
     ...
     where 
       ...
       liftToMod = do let (before, parent,after)=divideDecls (hsDecls mod) pnt                               
                      ...
                      let liftedDecls1=definingDecls [pn] parent True True
                          declaredPns=nub $ concatMap definedPNs liftedDecls1
                          liftedDecls = definingDecls declaredPns (hsDecls mod) True True
                      ...
                      if pns==[] 
                        then do (parent',liftedDecls',paramAdded)<-addParamsToParentAndLiftedDecl pn dd parent liftedDecls
                                ...
                                mod'<-moveDecl1 (replaceDecls mod (before++parent'++after))
                                       (Just (ghead "liftToMod" (definedPNs (ghead "liftToMod2" parent')))) declaredPns True -- [pn] True
...
addParamsToParentAndLiftedDecl pn dd parent liftedDecls 
  =do  (ef,ed)<-hsFreeAndDeclaredPNs parent
       (lf,ld)<-hsFreeAndDeclaredPNs liftedDecls   
