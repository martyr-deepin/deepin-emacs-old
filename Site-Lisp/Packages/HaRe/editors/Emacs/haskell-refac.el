...
(define-minor-mode haskell-refac-mode
  "Toggle Haskell Refactoring mode.
   ...
   (default /home/andy/MyEmacs/Site-Lisp/Packages/HaRe/refactorer/pfe, see option haskell-refac-refactorer).
   "
  ;; The initial value.
  nil
...
(defcustom haskell-refac-refactorer "/home/andy/MyEmacs/Site-Lisp/Packages/HaRe/refactorer/pfe"
	"*Path to Haskell refactoring tool"
	:type 'file
	:group 'haskell-refac)

(defcustom haskell-refac-chasePaths (cons (expand-file-name ".") '("/home/andy/MyEmacs/Site-Lisp/Packages/HaRe/tools/base/tests/HaskellLibraries"))
...
("Definitions"
   ...
   ["Transform pointwise to pointfree" haskell-refac-pwToPf  :active t]
...
("Data types"
   ...
   ["Convert a data type into a newtype" haskell-refac-refacDataNewType  :active t]
...
(defun haskell-refac-pwToPf ( start1 end1)
  "pwToPf"
  (interactive "r")
  (let (modified)
     (dolist (b (buffer-list) modified)
     ...
     (if modified (message-box (format "there are modified buffers: %s" modified))
       (process-send-string "haskell-refac-process" 
         ...
         "pwToPf " (mapconcat 'identity (append 
		(list (if (buffer-file-name) (buffer-file-name) "<missing filename>"))
		(list (number-to-string (line-no-pos start1))
		(number-to-string (+ 1 (current-column-pos start1)))
		(number-to-string (line-no-pos end1))
...
(defun haskell-refac-refacDataNewType ()
  "refacDataNewType"
  (interactive "")
  (let (modified)
     (dolist (b (buffer-list) modified)
     ...
     (if modified (message-box (format "there are modified buffers: %s" modified))
       (process-send-string "haskell-refac-process" 
         ...
         "refacDataNewType " (mapconcat 'identity (append 
		(list (if (buffer-file-name) (buffer-file-name) "<missing filename>"))
		(list (number-to-string (line-no)) (number-to-string (+ 1 (current-column))))) " ")
         "\n"))
     )
