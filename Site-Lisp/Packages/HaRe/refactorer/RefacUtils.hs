...
module RefacUtils(module Control.Monad.State, module StrategyLib, module RefacTypeSyn, module PosSyntax,
 ...
 -- * Program Analysis 
   ...
   ,exportInfo, isExported, isExplicitlyExported, modIsExported
    ...
    ,usedWithoutQual,canBeQualified, hasFreeVars,isUsedInRhs, isUsedInRhsName, pntPrefixOfExpr, getParams
    ,findPNT,findPN, findPN', findPNRHS      -- Try to remove this.
    ,findPNs, findEntity, findEntityWithLocation
    ,sameOccurrence   
    ,defines,definesTypeSig, isTypeSigOf, definedInPattern, definedInPatternPName, definesMatchOrPat, definesPNT
    ...
    -- ** Locations
    ,defineLoc, useLoc,locToPNT,locToPN,locToExp, locToExp2, locToPat, locToLocalPat, getStartEndLoc  
 ...
 -- * Program transformation 
    ...
    -- ** Rmoving
    ,rmDecl, rmTypeSig, commentOutTypeSig, insertComment, rmParams
...
-- * Miscellous
   ...
   ,parseSourceFile,parseSourceFile2, writeRefactoredFiles, showEntities ,showPNwithLoc 
    -- ** Locations
   ...
   ,defaultPN,defaultPNT,defaultModName,defaultExp,defaultPat
    ...
    ,pNTtoPN,pNTtoName,pNtoName,nameToPNT, nameToPN,pNtoPNT, declToName, declToPNT, declToPNTNoTypeSig, declToPName, declToPName2
    ,expToPNT, expToPN, nameToExp, nameToIdent,pNtoExp,patToPNT, patToPN, nameToPat,pNtoPat, typToPNT, nameToTyp
    ,definingDecls, definedPNs, definedPNsForConstr, extractPNT
   ...
   ,mkNewName, applyRefac, applyRefacToClientMods
    ...
    -- ** Type checking
    ,checkTypes, getDataName, checkTypesWithArity, checkTypesInPat, getTypes, getContext, cleanTypes, addTypeDecl, getSig, getSigAsString, getSigOmitLast, 
     nameToTypePNT, findType, createApplication, linesForType
 )
where
  ...
  addLocalDecl parent (newFun, newFunToks)
    =do
        ...
        let (startPos@(_,startCol),endPos'@(endRow',_))  --endPos' does not include the following newline or comment.
              =if localDecls==[] then startEndLocIncFowComment toks parent  --  startEndLoc toks parent    --The 'where' clause is empty
                                 else startEndLocIncFowComment toks localDecls  -- startEndLoc toks localDecls 
...
{-addTypeDecl::()
                    =>t                 -- ^ The AST.
                     -> Maybe PName     -- ^ If this is Just, then the declaration will be added righ after this identifiers' definition.
                    ->([HsDeclP], Maybe [PosToken])   -- ^ The type declaration to be added, in both AST and Token stream format (optional).
                    ->Bool              -- ^ if this is True, the declaration is a toplevel declaration
                    ->m t               -- ^ the result. -}
addTypeDecl::((MonadState (([PosToken],Bool),(Int,Int)) m), StartEndLoc t, HsDecls t, Printable t)
                    =>t                 -- ^ The AST.
                     -> Maybe PName    -- ^ If this is Just, then the declaration will be added righ after this identifiers' definition.
                    ->([HsDeclP], Maybe [PosToken]) -- ^ The type declaration to be added, in both AST and Token stream format (optional).
                    ->Bool             -- ^ if this is True, the declaration is a toplevel declaration
...
{- rmDecl::(MonadPlus m, MonadState (([PosToken],Bool),t1) m)
...
moveDecl pns dest sameLevel decls incSig
   = do ((ts,_),_)<-get  
        let (_, defToks') =getDeclAndToks (ghead "moveDecl:0" pns) True ts decls 
...
parseSourceFile2 filename modName
   = do
        name <- fileNameToModName filename 
        res <- ((checkScope  @@ parseModule') name)
        return res -- (checkScope @@ parseSourceFile') filename 
  where 
   checkScope (ts,(((wm,_),mod),refs))
...
declToPNTNoTypeSig (Dec (HsFunBind _ ((HsMatch _ pnt _ _ _):xs)))  
 = pnt
declToPNTNoTypeSig (Dec (HsPatBind _ pnt _ _)) = patToPNT pnt
declToPNTNoTypeSig (Dec (HsDataDecl _ _ t _ _)) = extractPNT t
declToPNTNoTypeSig _ = defaultPNT
...
{-|
    given a list of possible names, return the name of the declaration
    in PName format if it matches any of the names.
-}
...
{-|
    Given a PNT and an expression, return True if the PNT
    is a prefix of any of the names in the expression, otherise return False. 
 -}
pntPrefixOfExpr :: PNT -> HsExpP -> Bool
pntPrefixOfExpr pnt e@(Exp (HsApp e1 e2)) = (pntPrefixOfExpr pnt e1) || (pntPrefixOfExpr pnt e2)
pntPrefixOfExpr pnt e@(Exp (HsId (HsVar x))) = sameName pnt x
pntPrefixOfExpr pnt e@(Exp (HsTuple (e1:es))) = or ( map (pntPrefixOfExpr pnt) (e1:es)  )
pntPrefixOfExpr pnt e@(Exp (HsParen e1)) = pntPrefixOfExpr pnt e1
pntPrefixOfExpr pnt e@(Exp (HsCase e1 _)) = pntPrefixOfExpr pnt e1
pntPrefixOfExpr pnt e = False
...
getParams (Exp (HsApp (Exp (HsId (HsVar (PNT (PN (UnQual "unknown") (G (PlainModule "unknown") "--" (N Nothing))) Value (N Nothing)) ))) e2)) = [e2]
getParams e@(Exp (HsCase e1 _)) = getParams e1
getParams e@(Exp (HsApp e1 e2)) 
 = (getParams e1) ++ getParams2 e2
getParams (Exp (HsTuple (e:es))) 
 | getParams e /= [] = getParams e
 | otherwise = getParams (Exp (HsTuple es))
getParams e = []
...
{-|
   given a PName and a declaration, return True if the declaration is named after the PName
   If the function is a match, only look at the first entity.
 -}
definesMatchOrPat :: PName -> HsDeclP -> Bool
definesMatchOrPat (PN (UnQual x) s) decl@(Dec (HsFunBind loc ((HsMatch loc1 (PNT (PN (UnQual x2) s2) ty loc2) pats rhs ds):ms))) 
 = x == x2
definesMatchOrPat (PN (UnQual x) s) decl@(Dec (HsPatBind loc p rhs ds)) = x == pNTtoName (patToPNT p)
definesMatchOrPat _ _= False

{-|
   definedInPattern takes a PName, a list of patterns and a declaration.
   Return true whether the PName is defined within the given declaration.
    If the the declaration is a pattern, return True if either the PName is declared in the
    pattern or otherwise if the PName is defined is defined in the where clause of the pattern, 
    and the pattern is not defined as a tuple or as a literal. 
      
 -}
definedInPattern :: PName -> [HsPatP] -> HsDeclP -> Bool
definedInPattern pn@(PN (UnQual x) s) (p1:ps) decl@(Dec (HsFunBind loc ((HsMatch loc1 (PNT (PN (UnQual x2) s2) ty loc2) pats rhs ds):ms))) 
 = x == x2 || (or (map (definedInPattern pn (p1:ps)) ds))
definedInPattern pn@(PN (UnQual x1) s) (p1:ps) decl@(Dec (HsPatBind loc p rhs ds)) 
     = (elem x1 (map pNtoName (hsPNs p)) || (or (map (definedInPattern pn (p1:ps)) ds))) && and (litsInPat (p1:ps) [p])
definedInPattern _ _ _= False


litsInPat :: [HsPatP] -> [HsPatP] -> [Bool]
litsInPat [] _ = [True]
litsInPat _ [] = [True]
...
{-|
   definedInPatternPName takes a PName, a list of patterns and a declaration.
   If the PName has the name of the Declaration, then it is returned.
   If the PName is defined in the list of patterns, then the list of PNames in the where clause is returned.
-}
definedInPatternPName :: PName -> [HsPatP] -> HsDeclP -> [PName]
definedInPatternPName pn@(PN (UnQual x) s) (p:ps) decl@(Dec (HsFunBind loc ((HsMatch loc1 (PNT (PN (UnQual x2) s2) ty loc2) pats rhs ds):ms))) 
 | x == x2 = [(PN (UnQual x2) s2)]
 | (or (map (definedInPattern pn (p:ps)) ds)) = concat (map (definedInPatternPName pn (p:ps)) ds)
 | otherwise = []
definedInPatternPName pn@(PN (UnQual x1) s) (p1:ps1) decl@(Dec (HsPatBind loc p rhs ds)) 
 | elem x1 (map pNtoName (hsPNs p)) = (hsPNs p) ++ (hsPNs2 p)
 | (or (map (definedInPattern pn (p1:ps1)) ds)) = concat (map (definedInPatternPName pn (p1:ps1)) ds)
definedInPatternPName _ _ _ = []
...
getDeclAndToks pn incSig toks t
    = ghead "getDeclAndToks1" $ applyTU (stop_tdTU (failTU `adhocTU` inDecls)) t
  where 
    inDecls decls 
      |snd (break (defines pn) decls) /=[] 
    ...
    getDeclAndToks' pn incSig decls toks  
     = let (decls1', decls2') = break (defines pn) decls
           decl = if decls2' == [] then error "getDeclAndToks1:: declaration does not exisit"
                                   else ghead "getDeclAndToks1" decls2'
           declaredPns=definedPNs decl
           typeSig = if (not incSig)
                       then Nothing
                       else  let res = filter(\d-> any (\pn-> definesTypeSig pn d) declaredPns) decls
                             in if res==[] then Nothing
                                           else Just res                                              
           ...
           sigToks = case typeSig of 
                       ...
                       Just sigs ->
                           concatMap(\s -> case s of
                             (sig@(Dec (HsTypeSig _ [i] _ _)))-> removeOffset offset $ getToks' sig toks
                             (Dec (HsTypeSig loc is c ty))-> let sig' =(Dec (HsTypeSig loc0 [nameToPNT (pNtoName pn)] c ty))
...
checkTypes dat name ses modName = or (map (ordSubset dat) (tails (ghcTypeCheck1 ses name modName)))


{-| Given a function name, the ghc session and a module name, 
      return the type on the function in String format.
-}
getTypes :: String -> Session -> String ->  [String ]
getTypes name ses modName =  linesForType (ghcTypeCheck1 ses name modName)
...
{-|
    Given a type in string format, return a tuple where the first element is the
    context in string format, and the second element is the rest of the type signature.
 -}
...
{-|
    checkTypesWithArity is the same as checkTypes, the only difference is that it also
    returns which argument (the arity) is of the type in question.
...
checkTypesWithArity dat name ses modName
 | res /= []  = (True, res2)
 | otherwise  = (False, []) 
   where
     res = findArity (linesForType (ghcTypeCheck1 ses name modName)) dat
     (res2, _) = splitAt (length res -1) res
    
{-|
    checkTypesInPat is same as checkTypes, the only difference is that it converts
    the type in a pattern into a string, together with the arity of the type.
    The Bool represents that the argument is of the type in question.
-}    
checkTypesInPat :: String -> [HsPatP] -> Session -> String -> (Bool, [([Int], String)])     
checkTypesInPat dat pats ses modName
 = (False, (res (convertPats pats)))
    where
      res [] = [([],"")]
      res (x:xs)
       | x == "NULL" = ([0],"") : (res xs)
       | otherwise   = ((init (findArity (linesForType (ghcTypeCheck1 ses x modName)) dat) ), x) : (res xs)
...
{-|
    Given a string of the from t1 -> t2 -> t3 return the string
    in a format [t1, t2, t3] 
 -}
...
linesForType (x:xs)
  ...
  ls
    | xs'' == [] = []
    | otherwise = linesForType (tail xs'')


{-|
    given a term t, getDataName returns the name of the Type Constructor
    defined in t.
-}
...
{-|
    createDataFunc creates a type application
    of a given data constructor PNT and a list of types
 -}
...
{-| Given a session and a function name
     return the type in string format, the result is formatted
     so that if the context is empty the => is ommitted. CleanTypes is also
     called on the types.
 -}
getSigAsString :: Monad m => Session -> String -> String -> m String
getSigAsString ses modName name
  = do
      let types = getTypes name ses modName            
      -- error $ show types
      let types1 = cleanTypes (tail types)
   ...
   --   let context2 = init context
      ...
      return ((context2 context) ++  (head types3) ++ (types42 types4))  
        where 
         context2 [] = ""
         context2 x = x ++ " => "
         types42 [] = ""
...
{-| return the arguments in  a type dignature declaration format
     for a given function.
 -} 
getSigOmitLast :: Monad m => Session -> String -> String -> m HsDeclP
getSigOmitLast ses modName name 
 = do
      let types = getTypes name ses modName            
      -- error $ show types
      let types1 = cleanTypes (tail types)
...
{-| Given a type defined as a PNT and an AST return True if the type signature 
   defined in t defines the PNT.
-}
